...
BOOST_AUTO_TEST_CASE( constructorTest )
{   
    auto ai = std::make_shared<AI>(false, BLACK);
    for(int i = 0; i < 5; i++) 
    	ai->addPiece(std::make_shared<Piece>(i, i+1, WHITE, false, false));
    BOOST_CHECK_EQUAL(ai->getColor(), BLACK);
    BOOST_CHECK_EQUAL(ai->isActive(), false);
    BOOST_CHECK_EQUAL(ai->isWinner(), false);
}
...
BOOST_AUTO_TEST_CASE( settersTest )
{
    ...
    ai->setActive(true);
    ai->setWinner(true);
    BOOST_CHECK_EQUAL(ai->isActive(), true);
    BOOST_CHECK_EQUAL(ai->isWinner(), true);
    BOOST_CHECK_EQUAL(ai->getPiecesNumber(), 5);
}
...
{   
    ...
    BOOST_CHECK_EQUAL(game->isHumanTurn(), true);
    BOOST_CHECK_EQUAL(game->checkVictory(), false);
}

BOOST_AUTO_TEST_CASE( wrongMoveTest )
{   
    auto piece = std::make_shared<Piece>(5, 5, BLACK, false, false);
    auto move = std::make_shared<Move>(6, 5, false, piece);
    auto game = std::make_shared<Game>();
    BOOST_CHECK_EQUAL(game->checkMove(move), false);
}

BOOST_AUTO_TEST_CASE( rightMoveTest )
{   
    auto piece = std::make_shared<Piece>(5, 5, BLACK, false, false);
    auto move = std::make_shared<Move>(6, 6, false, piece);
    auto game = std::make_shared<Game>();
